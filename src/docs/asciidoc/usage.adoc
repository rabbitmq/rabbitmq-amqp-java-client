:test-examples: ../../test/java/com/rabbitmq/amqp1/docs
...
They all implement the {javadoc-url}/com/rabbitmq/amqp1/Resource.html[`Resource`] marker interface.
The {javadoc-url}/com/rabbitmq/amqp1/Resource.StateListener.html[`StateListener`] interface has only one method that takes a {javadoc-url}/com/rabbitmq/amqp1/Resource.Context.html[`Context`] argument.
See the snippet above for the information available in the `Context`.

A resource {javadoc-url}/com/rabbitmq/amqp1/Resource.State.html[`State`] can have the following values: `OPENING`, `OPEN`, `RECOVERING`, `CLOSING`, `CLOSED`.
