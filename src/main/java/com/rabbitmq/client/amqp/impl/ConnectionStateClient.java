...
final class ConnectionStateClient implements AutoCloseable {
  ...
  static class ConnectionState {
    ...
    private void handleDisconnect(long eventEpoch, DisconnectionEvent event) {
    ...
    private void handleNativeRecoverySuccess(
    ...
    private void handleTopologyRecoverySuccess(long attemptEpoch) {
      apply(
          onTopologyRecoverySuccess(
              this.internalState, this.epoch, attemptEpoch, this.connection.name()));
    }

    private void handleTopologyRecoveryFailure(long attemptEpoch) {
    ...
    static TransitionResult onDisconnect(
      ...
      return TransitionResult.of(
          ...
          s -> {
            ...
            s.connection.updateState(RECOVERING, exception);
            s.updateStateOfResources(RECOVERING, exception);
            s.connection.releaseManagementResources(exception);
            // Null out native resources
            s.connection.resetNativeResources();
    ...
    private static TransitionResult onTopologyRecoveryFailure(
      ...
      return TransitionResult.of(
          ...
          newEpoch,
          s -> {
            s.tearDownFailedNativeConnection();
            s.connection.resetNativeResources();
            s.connection.submitRecoveryTask(() -> s.connection.dispatchNativeRecovery(newEpoch));
          });
    }
  ...
  /** Outcome of transition (next internal state and epoch, side effect= */
  static final class TransitionResult {

    private static final Consumer<ConnectionState> NO_OP = s -> {};

    private final InternalState state;
    private final long epoch;
    private final Consumer<ConnectionState> effect;

    private TransitionResult(InternalState state, long epoch, Consumer<ConnectionState> effect) {
      this.state = state;
      this.epoch = epoch;
      this.effect = effect;
    }
    ...
    private static TransitionResult of(
        InternalState state, long epoch, Consumer<ConnectionState> effect) {
  ...
  /** Narrow view of the connection the state client manipulates. */
